  <div class="modal fade" id="inviteMembersModal" tabindex="-1" aria-labelledby="addmemberModalLabel" aria-hidden="true">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select type="text" class="form-control required select2" name='team_member[]' multiple id='team_member_member' required>
                                        ...
                                        @foreach($users as $user)
                                            <option value="{{$user->id}}" @foreach($task->users as $u) @if($user->id == $u->id) selected @endif @endforeach>{{$user->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            <!--end col-->
